...
For this repo the useful domains are `web` (React Native app-interface rules), `ux`, and `--stack react-native`; the `--design-system` generator is tuned for marketing landing pages and its pattern/style/typography output does not fit an existing branded mobile app. See `docs/ui-ux-audit-rider-core.md`.
