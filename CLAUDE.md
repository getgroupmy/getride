...
**Dispatch security (migration `0067`)**: `ride_requests` is participant-scoped — dispatch requires an authenticated Supabase session (legacy local-PIN sessions can't book real rides). Signed-in users see `open` requests plus rides they ride/drive; `admin_access` holders see everything. Riders insert only their own requests; `open` rows stay updatable by any authenticated user (claiming sets `partner_id` to the caller, offers keep the row open), and Postgres re-applies the SELECT policy to updated rows so a non-participant can never move a request into a state they can't see. One realtime caveat: a partner no longer receives the UPDATE event when someone else claims an open request — their incoming card dismisses via its countdown, and the atomic claim guard handles the accept race.
...
RLS is enabled by default, but many tables intentionally carry permissive/public write policies so the anon client can write directly (e.g. `app_settings`, `settings_entries`). Owner-scoped tables (like `profiles`) require `auth.uid()` to match the row; admin/back-office writes otherwise use the `service_role` key. Locked-down exceptions: `ride_requests` is participant-scoped since `0067` (see Ride Dispatch above) and `ip_access_rules` writes are admin-only (reads stay open for login-time blacklist checks); since `0066`, the wallet tables are read-only to clients (writes go through owner-scoped RPCs — see Wallets above), and the `app_settings` row `fare_ai_provider` (secret AI provider keys) is only visible/writable to profiles with an `admin_access` row or the service role — non-admin clients get fare estimates through the `ai-route-proxy` edge function instead.
...
- **Admin access guard**: Admin entry requires a Supabase-authenticated profile with at least one `admin_access` row — `admin-login` re-verifies the signed-in user's own login PIN via the rate-limited `verify_pin_for_login` RPC, and `admin-dashboard` bounces non-admins back to the login screen. There is no shared admin password in production; the legacy demo PIN/credentials (`admin`/`admin123`, PIN `522337`) work only in `__DEV__` builds, where they set a client-side god-mode flag that production builds ignore. Screens under the admin panel check `useAdminAccess()` (`canRead`/`canEdit` per page, `isAdmin` for entry). Seed the first admin with: `insert into public.admin_access (profile_id, page, access_level) values ('<uuid>', '*', 'edit');`
