...
RLS is enabled by default, but many tables intentionally carry permissive/public write policies so the anon client can write directly (e.g. `ride_requests`, `app_settings`, `settings_entries`). Owner-scoped tables (like `profiles`) require `auth.uid()` to match the row; admin/back-office writes otherwise use the `service_role` key. Two exceptions since migration `0066`: the wallet tables are read-only to clients (writes go through owner-scoped RPCs — see Wallets above), and the `app_settings` row `fare_ai_provider` (secret AI provider keys) is only visible/writable to profiles with an `admin_access` row or the service role — non-admin clients get fare estimates through the `ai-route-proxy` edge function instead.

**Admin-settings write gate (migration `0067`)**: `ip_access_rules`, `commission_rates`, and `get_coin_settings` are each trusted, unauthenticated, by a server-side consumer (the admin-login whitelist bypass, `wallet_charge_ride_commission`'s rate resolution, and `wallet_trade_coins`' peg anchoring respectively) — a public write policy on any of them is a path to a real exploit, not just data hygiene. Writes require `admin_write_access('<page-key>')`, which resolves to `admin_can_edit(auth.uid(), page)` for ordinary sessions and `true` for the service role / direct DB sessions; reads stay public since all three are read pre-login or from rider/partner-facing screens. When adding a new admin-only settings table whose data a server-side function later trusts, gate its writes the same way from the start rather than defaulting to `using (true)`.

**Legacy admin login bootstrap gate (migration `0068`)**: `app/admin-login.tsx` still carries hardcoded super-admin credentials (username/password, a PIN, and an IP-whitelist "Enter as admin" button) that set a client-only `ADMIN_SUPER_KEY` AsyncStorage flag with no corresponding `auth.uid()` — this predates the per-profile `admin_access` model. Since the very first `admin_access` row can only ever be inserted by the service role / a direct DB session (its own insert policy requires an existing admin), that legacy flow is the only way anyone reaches the admin panel before a real admin is seeded, so it can't simply be deleted without risking a lockout. Instead, `admin_access_exists()` (a public RPC) reports whether any real admin row exists yet; `AdminAccessContext.isLegacyAdminLoginAvailable()` wraps it (failing open if Supabase isn't configured or the RPC errors, matching the graceful-degradation convention below). `admin-login.tsx` and the whitelist-bypass shortcut in `MenuSideSheet.tsx` both check this before calling `markSuperAdminSession()` — once a real `admin_access` row exists anywhere, all three legacy entry points stop working and operators must sign in with their own admin-access-scoped Supabase session. Devices that already called `markSuperAdminSession()` before that point keep their local flag until they log out — there is no server-side session to revoke retroactively.
