...
import {
  ...
  setVehicleDocumentStatus,
  computeVehicleDocDisplayStatus,
  updateVehicleDocumentMetadata,
...
export default function AdminDocumentsVehiclesScreen() {
  const router = useRouter();
  const Colors = useColors();
  const [tab, setTab] = useState<ProviderDocStatus | "All">("Pending Review");
  const [rows, setRows] = useState<VehicleDocumentRow[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [refreshing, setRefreshing] = useState<boolean>(false);
  const [vehicleLabels, setVehicleLabels] = useState<Record<string, string>>({});
  const [selected, setSelected] = useState<VehicleDocumentRow | null>(null);
  ...
  const load = useCallback(async () => {
    const data = await fetchAllVehicleDocuments(tab === "All" ? null : tab);
    setRows(data);
    setLoading(false);

    const vehicleIds = Array.from(new Set(data.map((r) => r.vehicle_id))).filter(Boolean);
    if (vehicleIds.length > 0 && supabase) {
      const { data: vehicles } = await supabase
        .from("vehicle")
        .select("id, plate, make, model")
        .in("id", vehicleIds);
      const map: Record<string, string> = {};
      (vehicles ?? []).forEach((v: { id: string; plate: string | null; make: string | null; model: string | null }) => {
        const parts = [v.plate, [v.make, v.model].filter(Boolean).join(" ")].filter(Boolean);
        map[v.id] = parts.join(" • ") || v.id.slice(0, 8);
      });
  ...
  const handleDecide = async (status: "Approved" | "Rejected") => {
    ...
    setSaving(true);
    const updated = await setVehicleDocumentStatus(selected.id, status, notes.trim() || null);
    setSaving(false);
    if (!updated) {
  ...
  const handleSaveEdit = async (patch: EditableDocMetadata) => {
    if (!selected) return;
    setEditSaving(true);
    const updated = await updateVehicleDocumentMetadata(selected.id, patch);
    setEditSaving(false);
    if (!updated) {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    setDeleting(true);
    const ok = await deleteVehicleDocument(selected.id);
    setDeleting(false);
    if (!ok) {
  ...
  const openDetail = (row: VehicleDocumentRow) => {
    setSelected(row);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Colors.background }]} edges={["top", "bottom"]}>
      ...
      {loading ? (
        ...
        >
          {rows.length === 0 ? (
            ...
            rows.map((row) => {
              const s = computeVehicleDocDisplayStatus(row);
              const Icon = statusIcon(s);
              const color = statusColor(s);
              const label = vehicleLabels[row.vehicle_id] ?? row.vehicle_id.slice(0, 8);
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.cardInfo}>
                    ...
                    <Text style={[styles.cardSub, { color: Colors.textSecondary }]} numberOfLines={1}>{label}</Text>
      ...
      <VehicleDocReviewModal
        visible={!!selected}
        row={selected}
        vehicleLabel={selected ? vehicleLabels[selected.vehicle_id] ?? "" : ""}
        notes={notes}
...
function VehicleDocReviewModal({
  visible,
  row,
  vehicleLabel,
  notes,
        ...
        <View style={[styles.modalHeader, { borderBottomColor: Colors.border }]}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={[styles.headerSubtitle, { color: Colors.textSecondary }]} numberOfLines={1}>
              {vehicleLabel || row.vehicle_id.slice(0, 8)} • {s}
            </Text>
          </View>
          <TouchableOpacity
            onPress={onEdit}
