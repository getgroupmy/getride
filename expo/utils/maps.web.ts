...
export const MarkerAnimated = null;
...
import { runWithMappingRotation, getMappingKey } from "./mappingClient";
...
async function calculateRouteOSRM(
  ...
  try {
    ...
    console.log("OSRM (OpenStreetMap) Directions request (web)");
...
export async function calculateRoute(
  ...
  console.log("OSRM unavailable, falling back to Google Directions (web)");
  ...
  const doFetch = async (key: string) => {
    const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${originStr}&destination=${destStr}${waypointsStr}&mode=driving&key=${key}`;
    console.log("Google Directions request (web)");
    const response = await fetch(url, { headers: { 'Accept': 'application/json' } });
...
export function calculateFare(distanceKm: number, durationMin: number, multiplier: number = 1): number {
  const baseFare = 3;
  const perKm = 1.5;
  const perMin = 0.15;
  const fare = (baseFare + distanceKm * perKm + durationMin * perMin) * multiplier;
  return Math.max(5, parseFloat(fare.toFixed(2)));
}
...
export async function reverseGeocode(
  ...
  let GOOGLE_API_KEY = FALLBACK_KEY;
  try {
    const k = await getMappingKey(pageId, "geocoding");
    if (k?.key && k.key.trim().length > 0) GOOGLE_API_KEY = k.key;
  } catch {}
